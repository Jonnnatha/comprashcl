        ...
        <nav class="navbar navbar-main navbar-expand-lg px-0 mx-4 shadow-none border-radius-xl " id="navbarBlur"
            ...
            <div class="container-fluid py-1 px-3">
                <nav aria-label="breadcrumb">
                    ...
                    <h6 class="font-weight-bolder text-white mb-0">Cadastrar Compras - {{ $user->nome }}</h6>
                </nav>
        ...
        <div style="margin-top: 100px;" >
            ...
            <div class="row mt-3">
                                ...
                                <div class="col-md-6 mb-3">
                                    ...
                                    <div>
                                        <input type="checkbox" name="referencia[]" value="{{ $referencia->id }}">
                                        <label>{{ $referencia->referencia }}</label>
                                    </div>
                                @endforeach
                                </div>
                                <div class="row mt-3">
                                    ...
                                    <tbody>
                                        @foreach ($solicitante as $li)
                                                <tr id="{{ $li->id }}">
                                                    ...
                                                    <td>{{ $li->nome }}</td>
                                                    <td>*****</td>
                                                    <td>  @foreach($referenciasIds as $lo)
                                                       @if($lo->user_id == $li->id)
                                                    @foreach($referencias as $la)
                                                        @if($la->id == $lo->referencia_id)
                                                        {{$la->referencia}}/
                                                        @endif
                                                    @endforeach
          ...
          <div class="modal-content">
            <div class="modal-body p-0">
              <div class="card card-plain">
                ...
                <div class="card-body">
                    <form class="needs-validation" action="{{ route('compras.update', ['id' => $li->id]) }}" method="POST" novalidate>
                        @csrf
                        @method('PUT')
                            ...
                            <div class="multisteps-form__content">
                                <div class="row mt-3">
                                    ...
                                    <div class="col-md-12 mb-3">
                                        ...
                                        @foreach($referencias as $referencia)
                                            <div>
                                                <input type="checkbox" name="referencia[]" value="{{ $referencia->id }}" {{ in_array($referencia->id, $li->referencias->pluck('id')->toArray()) ? 'checked' : '' }}>
                                                <label>{{ $referencia->referencia }}</label>
                                            </div>
                                        @endforeach
                                      </div>
